...
from zebra_patch import ZebraForCausalLM
...
class Model:
    ...
    def post_init(self, model_name: str) -> None:
        if "zebra" in model_name:
            self.is_encoder_decoder = LlamaConfig.from_pretrained(model_name).is_encoder_decoder
            self.generation_config = GenerationConfig.from_model_config(LlamaConfig(model_name))
        else:
    ...
    def get_generation_config(self, request: GenerateRequest):
    ...
    def generate(self, request: GenerateRequest):
        try:
            ...
            exception.is_encoder_decoder = self.is_encoder_decoder
            return exception

    def forward(self, request: ForwardRequest):
...
def get_hf_model_class(model_class: str):
